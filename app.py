...
def round_endpoint():
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            app.logger.debug('POST /round - raw JSON: {data}')
...
def init_db():
    ...
    with app.app_context():
        try:
            ...
            # NEW: Golf holes table
            conn.run('''
                CREATE TABLE IF NOT EXISTS "golfHoles" (
                    id INTEGER GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
                    "courseID" INTEGER REFERENCES golfcourse(id) ON DELETE CASCADE,
            ...
            # NEW: Golf rounds table
            conn.run('''
                CREATE TABLE IF NOT EXISTS "golfRounds" (
                    id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
                    "playerID" INTEGER NOT NULL REFERENCES users(id) ON DELETE CASCADE,
                    "courseID" INTEGER NOT NULL REFERENCES golfcourse(id) ON DELETE RESTRICT,
                    "roundStartTimestamp" TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                    "totalScore" INTEGER
                )
            ''')
            
            # NEW: Golf round holes table (hole-by-hole performance)
            conn.run('''
                CREATE TABLE IF NOT EXISTS "golfRoundsHoles" (
                    id INTEGER GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
                    "roundID" INTEGER NOT NULL REFERENCES "golfRounds"(id) ON DELETE CASCADE,
                    "holeID" INTEGER NOT NULL REFERENCES "golfHoles"(id) ON DELETE RESTRICT,
                    score INTEGER,
                    "pathGeoJSON" JSONB,
                    samples TEXT,
                    "fairwayHit" BOOLEAN,
            ...
            conn.run('''
                CREATE TABLE IF NOT EXISTS greens (
                    id INTEGER GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
                    "holeID" INTEGER NOT NULL REFERENCES "golfHoles"(id) ON DELETE CASCADE,
            ...
            conn.run('CREATE INDEX IF NOT EXISTS idx_golfholes_courseid ON "golfHoles"("courseID")')
            conn.run('CREATE INDEX IF NOT EXISTS idx_golfrounds_playerid ON "golfRounds"("playerID")')
            conn.run('CREATE INDEX IF NOT EXISTS idx_golfrounds_courseid ON "golfRounds"("courseID")')
            conn.run('CREATE INDEX IF NOT EXISTS idx_golfroundsholes_roundid ON "golfRoundsHoles"("roundID")')
            conn.run('CREATE INDEX IF NOT EXISTS idx_golfroundsholes_holeid ON "golfRoundsHoles"("holeID")')
            conn.run('CREATE INDEX IF NOT EXISTS idx_greens_holeid ON greens("holeID")')
            conn.run('CREATE INDEX IF NOT EXISTS idx_hazards_holeid ON hazards("holeID")')
